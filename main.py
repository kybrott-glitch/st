...
def make_text_layers(text: str, color: list) -> list:
    ...
    anim.frame_rate = 60; anim.in_point = 0; anim.out_point = 360
    ...
    # FIX: also set ip/op/st — otherwise logo disappears after 1-2 frames
    for lyr in layers:
...
def make_svg_layers(svg_bytes: bytes) -> list:
    ...
    for lyr in src_layers:
        ...
        # FIX: ip/op/st — otherwise logo disappears after 1-2 frames
        l["ip"] = ip
        l["op"] = op
        l["st"] = 0
        new_layers.append(l)
...
def build_anim(anim_data: dict, logo_layers: list, extra_assets: list,
    ...
    # 1. Update the mylogo asset
    logo_asset = next((a for a in out.get("assets", [])
                       if a.get("id") == LOGO_ID), None)
    if logo_asset is None:
        out.setdefault("assets", []).append({"id": LOGO_ID, "layers": []})
        logo_asset = out["assets"][-1]

    new_layers = copy.deepcopy(logo_layers)

    # 2. Check if BA0047/FF4A52 colors are present
    t_ba = hex_to_rgba(COLOR_BA)
    t_ff = hex_to_rgba(COLOR_FF)
    has_ba = color_exists(out, t_ba)
    has_ff = color_exists(out, t_ff)
    has_special = has_ba or has_ff

    # 3. Logo color: logo_c1 if special colors exist, logo_c2 otherwise
    logo_color = logo_c1 if has_special else logo_c2
    if logo_color:
        recolor_logo(new_layers, logo_color)

    # 4. Scale + Anchor Point + Position + ip/op/st (for all layers)
    ...
    for lyr in new_layers:
        ks = lyr.setdefault("ks", {})
        # Anchor Point: center in layer's own coordinate system
        ks["a"] = {"a": 0, "k": [cx, cy, 0]}
        # Position: center in composition
        ks["p"] = {"a": 0, "k": [cx, cy, 0]}
        # Scale
        ks["s"] = {"a": 0, "k": [scale, scale, 100]}
        # FIX: sync ip/op/st to animation frame range
        # Without this, logo only shows for 1-2 frames then disappears
        lyr["ip"] = ip
        lyr["op"] = op
        lyr["st"] = 0

    logo_asset["layers"] = new_layers
    ...
    # 6. Replace BA0047 and FF4A52 colors
    if ba_color and has_ba:
        replace_color(out, t_ba, ba_color)
    if ff_color and has_ff:
        replace_color(out, t_ff, ff_color)
...
def protect_json(d: dict) -> dict:
    """
    Adds an invisible fake layer — with 'шарes' (Cyrillic а,р).
    Lottie/Telegram: invisible (opacity=0), animation unchanged.
    Bodymovin: throws 'Cannot read properties of undefined (reading __created)' error.
    """
    fake_layer = {
        "ddd": 0,
        "ind": 999,
        ...
        "ao": 0,
        "sh\u0430\u0440es": [{"ty": "gr", "it": [], "nm": "."}],  # kirill а(0430) р(0440)
        "ip": d.get("ip", 0),
        "op": d.get("op", 60),
        "st": 0,
        "bm": 0
    }
    # Insert at the beginning of assets[0].layers
    if d.get("assets") and isinstance(d["assets"], list) and len(d["assets"]) > 0:
        if isinstance(d["assets"][0].get("layers"), list):
            d["assets"][0]["layers"].insert(0, copy.deepcopy(fake_layer))
    # Also insert at the beginning of the main layers list
...
async def cmd_start(msg: Message, state: FSMContext):
    if msg.from_user.id != ALLOWED_USER:
        await msg.answer("⛔️ Access denied.")
        return
    await state.clear()
    await msg.answer(
        "👋 <b>Animated Emoji Pack Bot</b>\n\n"
        "📋 Which animations do you want to create?\n"
        "<i>Enter numbers separated by dots: <code>1.9.43.54</code>\n"
        "Or press the FULL button (65 items).</i>",
        reply_markup=select_kb(), parse_mode="HTML")
    await state.set_state(S.select)


@dp.message(S.select)
async def got_select(msg: Message, state: FSMContext):
    if msg.from_user.id != ALLOWED_USER:
        return
    indices = parse_indices(msg.text.strip())
    if not indices:
        await msg.answer(
            "⚠️ Invalid format.\n"
            "<i>Example: <code>1.9.43.54</code> or press the FULL button.</i>",
            parse_mode="HTML")
        return
    await state.update_data(selected=indices)
    await msg.answer("✅ Selected: " + ", ".join(f"{n:03d}" for n in indices) +
                     "\n\nChoose logo type:", reply_markup=main_kb(), parse_mode="HTML")
    await state.set_state(S.type)


@dp.callback_query(S.select, F.data == "full")
async def cb_full(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ALLOWED_USER:
        await c.answer("⛔️ Access denied."); return
    # FIX: range(1, 65) = 64 items, range(1, 66) = 65 items
    await state.update_data(selected=list(range(1, 66)))
    await c.message.edit_text(
        "✅ FULL selected (65 items)\n\nChoose logo type:",
        reply_markup=main_kb(), parse_mode="HTML")
    await state.set_state(S.type)
    await c.answer()


@dp.callback_query(S.type, F.data.in_({"json", "text", "svg"}))
async def cb_type(c: CallbackQuery, state: FSMContext):
    if c.from_user.id != ALLOWED_USER:
        await c.answer("⛔️ Access denied."); return
    await state.update_data(mode=c.data)
    await c.message.edit_text(
        f"🎨 Enter a new color for <b>BA0047</b> (stroke/fill):\n"
        f"<i>To keep unchanged: <code>skip</code></i>",
        parse_mode="HTML")
...
async def got_ba(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"🎨 Enter a new color for <b>FF4A52</b> (stroke/fill):\n"
        f"<i>To keep unchanged: <code>skip</code></i>",
        parse_mode="HTML")
...
async def got_ff(msg: Message, state: FSMContext):
    ...
    if d["mode"] == "json":
        ...
        await msg.answer("✏️ Enter the emoji text:\n<i>Example: KXALIL</i>", parse_mode="HTML")
...
async def got_json(msg: Message, state: FSMContext):
    ...
    if not (msg.document.file_name or "").endswith(".json"):
        await msg.answer("⚠️ Only <b>.json</b> files are accepted!", parse_mode="HTML"); return
    try:
...
@dp.message(S.json_file)
async def json_no_file(msg: Message):
    await msg.answer("⚠️ Please send a .json file!")
...
async def got_svg(msg: Message, state: FSMContext):
    ...
    if not fname.lower().endswith(".svg"):
        await msg.answer("⚠️ Only <b>.svg</b> files are accepted!", parse_mode="HTML"); return
    try:
        f = await bot.get_file(msg.document.file_id)
        buf = io.BytesIO(); await bot.download_file(f.file_path, buf); buf.seek(0)
        svg_bytes = buf.read()
    ...
    try:
        layers = await loop.run_in_executor(None, lambda: make_svg_layers(svg_bytes))
    except Exception as e:
        await msg.answer(f"❌ SVG parse error: <code>{e}</code>", parse_mode="HTML"); return

    await state.update_data(layers=layers, extra=[])
    await ask_logo_c1(msg, state)


@dp.message(S.svg_file)
async def svg_no_file(msg: Message):
    await msg.answer("⚠️ Please send a .svg file!")
...
async def got_text(msg: Message, state: FSMContext):
    if msg.from_user.id != ALLOWED_USER: return
    t = msg.text.strip()
    if not t: await msg.answer("⚠️ Text cannot be empty!"); return
    await state.update_data(user_text=t, layers=None, extra=[])
    await ask_logo_c1(msg, state)
...
async def ask_logo_c1(msg: Message, state: FSMContext):
    await msg.answer(
        f"🎨 Enter <b>Logo color 1</b>:\n"
        f"<i>Used in animations <b>with</b> BA0047/FF4A52\n"
        f"skip → keep unchanged</i>",
        parse_mode="HTML")
...
async def got_logo_c1(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"🎨 Enter <b>Logo color 2</b>:\n"
        f"<i>Used in animations <b>without</b> BA0047/FF4A52\n"
        f"skip → keep unchanged</i>",
        parse_mode="HTML")
...
async def got_logo_c2(msg: Message, state: FSMContext):
    ...
    # In text mode, layers haven't been created yet — build them now
    d = await state.get_data()
    if d.get("layers") is None:
        ...
        try:
            layers = await loop.run_in_executor(
                None, lambda: make_text_layers(d["user_text"], logo_c1))
        except Exception as e:
            await msg.answer(f"❌ Text error: <code>{e}</code>", parse_mode="HTML"); return
        await state.update_data(layers=layers, logo_c2=color)
...
async def send_preview(msg: Message, state: FSMContext):
    ...
    anim = get_001()
    if not anim:
        await msg.answer("❌ No files found in the lotties/ folder!"); return
    modified = build_anim(
        anim, d["layers"], d.get("extra", []),
        d.get("scale", 100.0),
        d.get("ba_color"), d.get("ff_color"),
        d.get("logo_c1"), d.get("logo_c2"))
    tgs = to_tgs(modified)
    await msg.answer_document(
        BufferedInputFile(tgs, filename="preview.tgs"),
        caption=(
            f"👆 <b>Preview</b> — scale: <b>{d.get('scale', 100.0):g}%</b>\n\n"
            f"✏️ Adjust: <code>+10</code>, <code>-5</code>, <code>120</code>\n"
            f"✅ Confirm: <code>DONE</code>"
        ), parse_mode="HTML")
...
async def scale_input(msg: Message, state: FSMContext):
    ...
    try:
        ...
        if new_s <= 0: raise ValueError
    except:
        await msg.answer(
            "⚠️ Invalid value.\n"
            "<i>Examples: <code>+10</code>, <code>-5</code>, <code>120</code>, <code>DONE</code></i>",
...
async def run_pack(msg: Message, d: dict):
    ...
    name = f"pk{suf}_by_{me.username}"
    titl = f"Pack {suf.upper()}"

    stat = await msg.answer(f"⚙️ Processing...\n<code>{name}</code>", parse_mode="HTML")

    if not LOTTIES_DIR.exists():
        await stat.edit_text("❌ <code>lotties/</code> folder not found!", parse_mode="HTML")
        return

    all_files = sorted(LOTTIES_DIR.glob("*.json"))
    if not all_files:
        await stat.edit_text("❌ No JSON files in <code>lotties/</code>!", parse_mode="HTML")
        return
    ...
    if not files:
        await stat.edit_text("❌ Selected files not found!", parse_mode="HTML")
        return

    created = False; ok = 0; bad = 0
    ...
    for i, fp in enumerate(files):
        try:
            ...
            tgs = to_tgs(modified)
            inp = BufferedInputFile(tgs, filename=f"e{i+1:03d}.tgs")
            sd  = {"sticker": inp, "emoji_list": ["⭐️"], "format": "animated"}

            if not created:
                await bot.create_new_sticker_set(
                    user_id=uid, name=name, title=titl,
            ...
            if (i + 1) % 10 == 0:
                await stat.edit_text(
                    f"⚙️ {i+1}/{len(files)} ✅  <code>{name}</code>", parse_mode="HTML")
            ...
            if not created:
                await stat.edit_text(
                    f"❌ Failed to create pack:\n<code>{e}</code>\n\n"
                    "💡 <code>custom_emoji</code> requires a special Telegram permission.\n"
                    "If unavailable: change to <code>sticker_type='regular'</code>.",
                    parse_mode="HTML")
                return
            bad += 1
    ...
    if created:
        await stat.edit_text(
            ...
            f"✔️ {ok} added" + (f"\n⚠️ {bad} failed" if bad else ""),
            parse_mode="HTML")
    else:
        await stat.edit_text("❌ Failed to create the pack.")


# ── Run ───────────────────────────────────────────────────────────────────────
async def main():
    logger.info(f"Bot started | Font: {DEFAULT_FONT}")
